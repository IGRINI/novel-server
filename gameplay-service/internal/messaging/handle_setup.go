...
func (p *NotificationProcessor) handleNovelSetupNotification(ctx context.Context, notification sharedMessaging.NotificationPayload, publishedStoryID uuid.UUID) error {
	...
	if notification.Status == sharedMessaging.NotificationStatusSuccess {
		...
		// Определение ожидаемого количества NPC на основе раздела chars:
		var expectedNpcCount int
		{
			var inChars bool
			var blockLines int
			for _, rawLine := range strings.Split(rawGeneratedText, "\n") {
			...
			if inChars && blockLines > 0 {
				expectedNpcCount++
			}
		}
		parsedSetup, errParseSetup := schemas.ParseNovelSetupPlain(rawGeneratedText, expectedNpcCount)
		if errParseSetup != nil {
			p.logger.Error("Failed to parse novel setup plain text", zap.String("task_id", taskID), zap.String("published_story_id", publishedStoryID.String()), zap.Error(errParseSetup))
			parseErr = fmt.Errorf("failed to parse novel setup output: %w", errParseSetup)

			errDetails := parseErr.Error()
			dbCtxUpdateStory, cancelUpdateStory := context.WithTimeout(ctx, 10*time.Second)
			defer cancelUpdateStory()
			if errUpdateStory := p.publishedRepo.UpdateStatusFlagsAndDetails(dbCtxUpdateStory, p.db, publishedStoryID, sharedModels.StatusError, false, false, &errDetails); errUpdateStory != nil {
				p.logger.Error("CRITICAL ERROR: Failed to update PublishedStory status to Error after parsing setup error",
					zap.String("task_id", taskID),
					zap.String("published_story_id", publishedStoryID.String()),
					zap.Error(errUpdateStory),
				)
			} else {
				p.logger.Info("PublishedStory status updated to Error due to setup parsing error",
					zap.String("task_id", taskID),
					zap.String("published_story_id", publishedStoryID.String()),
				)
			}
		...
		// Маршалинг parsedSetup в JSON
		setupContent := parsedSetup
		setupBytes, errMarshalSetup := json.Marshal(parsedSetup)
		if errMarshalSetup != nil {
			p.logger.Error("Failed to marshal parsed novel setup", zap.String("task_id", taskID), zap.String("published_story_id", publishedStoryID.String()), zap.Error(errMarshalSetup))
			parseErr = fmt.Errorf("failed to marshal parsed setup: %w", errMarshalSetup)

			errDetails := parseErr.Error()
			dbCtxUpdateStory, cancelUpdateStory := context.WithTimeout(ctx, 10*time.Second)
			defer cancelUpdateStory()
			if errUpdateStory := p.publishedRepo.UpdateStatusFlagsAndDetails(dbCtxUpdateStory, p.db, publishedStoryID, sharedModels.StatusError, false, false, &errDetails); errUpdateStory != nil {
				p.logger.Error("CRITICAL ERROR: Failed to update PublishedStory status to Error after marshaling setup error",
					zap.String("task_id", taskID),
					zap.String("published_story_id", publishedStoryID.String()),
					zap.Error(errUpdateStory),
				)
			} else {
				p.logger.Info("PublishedStory status updated to Error due to setup marshaling error",
					zap.String("task_id", taskID),
					zap.String("published_story_id", publishedStoryID.String()),
				)
			}
		...
		p.logger.Info("Successfully parsed and marshaled setup plain text", zap.String("task_id", taskID))
