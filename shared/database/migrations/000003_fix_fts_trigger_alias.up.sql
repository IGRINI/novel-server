...
-- Fixes FTS trigger function by renaming the alias 'c' to 'char_element'
...
BEGIN
    ...
    -- Extract data from setup JSONB (characters) - Renamed alias c to char_element
    SELECT string_agg(COALESCE(char_element->>'n', '') || ' ' || COALESCE(char_element->>'d', '') || ' ' || COALESCE(char_element->>'p', ''), ' ')
    INTO v_char_details
    FROM jsonb_array_elements(v_setup->'chars') AS char_element; -- Renamed alias here
