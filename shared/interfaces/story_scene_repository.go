...
type StorySceneRepository interface {
	...
	Create(ctx context.Context, querier DBTX, scene *models.StoryScene) error
	...
	FindByStoryAndHash(ctx context.Context, querier DBTX, publishedStoryID uuid.UUID, stateHash string) (*models.StoryScene, error)

	// GetByID retrieves a story scene by its unique ID.
	// Returns models.ErrNotFound if the scene is not found.
	GetByID(ctx context.Context, querier DBTX, id uuid.UUID) (*models.StoryScene, error)

	// GetByStoryIDAndStateHash retrieves a story scene by its story ID and state hash.
	// Returns models.ErrNotFound if the scene is not found.
	GetByStoryIDAndStateHash(ctx context.Context, querier DBTX, storyID uuid.UUID, stateHash string) (*models.StoryScene, error)

	// ListByStoryID retrieves all scenes for a given story.
	// Returns models.ErrNotFound if no scenes exist for the story.
	ListByStoryID(ctx context.Context, querier DBTX, storyID uuid.UUID) ([]models.StoryScene, error)
