...
type PlayerProgressRepository interface {
	...
	// Returns models.ErrNotFound if no progress exists for the given user and story.
	GetByUserIDAndStoryID(ctx context.Context, userID, publishedStoryID uuid.UUID) (*models.PlayerProgress, error)
	...
	Delete(ctx context.Context, userID, publishedStoryID uuid.UUID) error
}
