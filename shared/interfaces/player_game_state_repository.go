...
type PlayerGameStateRepository interface {
	...
	ListByPlayerAndStory(ctx context.Context, querier DBTX, playerID uuid.UUID, publishedStoryID uuid.UUID) ([]*models.PlayerGameState, error)
	...
	GetByID(ctx context.Context, querier DBTX, gameStateID uuid.UUID) (*models.PlayerGameState, error)
	...
	Save(ctx context.Context, querier DBTX, state *models.PlayerGameState) (uuid.UUID, error)
	...
	DeleteByPlayerAndStory(ctx context.Context, querier DBTX, playerID uuid.UUID, publishedStoryID uuid.UUID) error
	...
	CheckGameStateExistsForStories(ctx context.Context, querier DBTX, playerID uuid.UUID, storyIDs []uuid.UUID) (map[uuid.UUID]bool, error)

	// ListByStoryID получает все состояния игры для указанной истории (для админ целей?).
	ListByStoryID(ctx context.Context, querier DBTX, publishedStoryID uuid.UUID) ([]models.PlayerGameState, error)
	...
	Delete(ctx context.Context, querier DBTX, gameStateID uuid.UUID) error
