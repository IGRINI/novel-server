...
*   **`POST /api/published-stories/:story_id/retry`**
    *   Описание: Повторный запуск **начальной** задачи генерации (Setup или самой первой сцены) для опубликованной истории, которая завершилась с ошибкой (`status: "error"`). Этот эндпоинт **не** ретраит сцены после первой.
    *   Аутентификация: **Требуется.**
    *   Параметр пути: `:story_id` - UUID опубликованной истории (`PublishedStory`).
    *   Тело запроса: Нет.
