...
-- Создаем временную таблицу без user_id в первичном ключе
...
-- Копируем уникальные данные по хешу
...
SELECT DISTINCT ON (novel_id, scene_index, state_hash) 
    novel_id, scene_index, user_id, state_hash, state_data, created_at, NOW() as updated_at
FROM novel_states
WHERE state_hash IS NOT NULL AND state_hash != '';
...
-- Создаем индекс для быстрого поиска последнего состояния пользователя
CREATE INDEX idx_novel_states_user_progress ON novel_states(novel_id, user_id, scene_index DESC);

-- +migrate Down

-- Восстанавливаем исходную структуру таблицы
CREATE TABLE novel_states_old (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (novel_id, scene_index, user_id)
);

-- Копируем данные обратно
INSERT INTO novel_states_old (novel_id, scene_index, user_id, state_hash, state_data, created_at, updated_at)
...
-- Создаем индексы
CREATE INDEX idx_novel_states_novel_id_scene ON novel_states(novel_id, scene_index DESC);
